...
> {
    ...
    > {
        ...
        asyncHandler.setOptionalAddressFieldsFn =
            this.setOptionalAddressFieldsFn
    ...
    async handle(
        ...
        if (this instanceof EmailEventHandlerWithAsyncData) {
            try {
                ;(event as EventWithAsyncData<Event, any>).data =
                    await this._loadDataFn({
                        event,
                        injector,
                    })
            } catch (err: unknown) {
    ...
    private getBestConfiguration(
        ...
        const channelMatch = this.configurations.find(
            (c) =>
                c.channelCode === channelCode && c.languageCode === 'default',
        )
        if (channelMatch) {
            return channelMatch
        }
