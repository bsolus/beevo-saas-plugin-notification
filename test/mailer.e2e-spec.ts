...
import { orderConfirmationHandler } from '../src/mailer/default-notification-handlers'
import { EmailSender } from '../src/mailer/email-sender'
import { EmailEventHandler } from '../src/mailer/event-handler'
import { EmailEventListener } from '../src/mailer/event-listener'
...
describe('EmailPlugin', () => {
    ...
    async function initPluginWithHandlers(
        ...
        options?: Partial<NotificationPluginOptions>,
    ) {
        ...
        module = await Test.createTestingModule({
            imports: [
                ...
                TypeOrmModule.forFeature([EmailPartial]),
                PluginCommonModule,
            ...
            providers: [EmailPartialService, EmailTemplateService, MockService],
        }).compile()

        Logger.useLogger(testingLogger)
        module.useLogger(new Logger())

        const plugin = module.get(NotificationPlugin)
    ...
    describe('Dynamic transport settings', () => {
        ...
        it('Initializes with async transport settings', async () => {
            ...
            expect(
                typeof (module.get(NotificationPlugin) as any).options
                    .transport,
            ).toBe('function')
        })
