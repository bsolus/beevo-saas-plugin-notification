...
import { AdminNotificationResolver } from './src/api/admin/notification.resolver'
import { adminApiExtensions } from './src/api/api-extensions'
import { EmailPartialTranslation } from './src/entities/email-partial-translation.entity'
...
@VendurePlugin({
    ...
    providers: [
        ...
        {
            provide: NOTIFICATION_PLUGIN_OPTIONS,
            useFactory: () => NotificationPlugin.options,
        },
...
{
    private static options: InitializedNotificationPluginOptions
    private devMailbox: DevMailbox | undefined
    private jobQueue: JobQueue<IntermediateEmailDetails> | undefined
    private testingProcessor: NotificationProcessor | undefined
    ...
    private async setupEventSubscribers() {
        for (const handler of NotificationPlugin.options.handlers) {
